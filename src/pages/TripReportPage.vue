...
onMounted(async () => {
  ...
  const map = L.map('map').setView(countriesCoordinates[countryName.value] || [51.505, -0.09], 5)
  ...
  <div class="p-8 space-y-6 bg-background dark:bg-gray-900 rounded-lg shadow-md">
      ...
      <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200 flex items-center">
        ...
        >
          <path
            ...
            stroke-width="2"
            d="M12 2C9.243 2 7 4.243 7 7c0 1.157.488 2.198 1.272 2.928L12 15l3.728-5.072A3.001 3.001 0 0017 7c0-2.757-2.243-5-5-5zM12 19v3"
          />
        </svg>
        Destination
      </h2>
      <p class="text-lg font-medium text-gray-800 dark:text-gray-300">
    ...
    <div class="p-4 bg-white dark:bg-gray-700 rounded shadow-md">
      <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200 flex items-center">
        ...
        >
          <path
            ...
            stroke-linejoin="round"
            stroke-width="2"
            d="M12 8c3.866 0 7-1.343 7-3M5 8c0 1.657 3.134 3 7 3s7-1.343 7-3M5 8v4c0 1.657 3.134 3 7 3s7-1.343 7-3V8M5 12v4c0 1.657 3.134 3 7 3s7-1.343 7-3v-4"
          />
        </svg>
        Budget Info
      </h2>
      <p class="dark:text-gray-200">
        Your maximum budget:
        <span class="font-semibold dark:text-gray-200">{{ maxBudget }} {{ currentCurr }}</span>
      </p>
      <p class="dark:text-gray-200">
        Converted budget:
        <span class="font-semibold dark:text-gray-200"
          >{{ newMaxBudget }} {{ destinationCurr }}</span
        >
      </p>
      <p class="dark:text-gray-200">
        Budget per person:
        <span class="font-semibold dark:text-gray-200"
          >{{ (newMaxBudget / travelers).toFixed(2) }} {{ destinationCurr }}</span
        >
      </p>
      <p class="dark:text-gray-200">
        Average daily cost of living in {{ countryName }}:
        <span class="font-semibold dark:text-gray-200">{{ dailyCost }} {{ destinationCurr }}</span>
      </p>
    </div>
      ...
      <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200 flex items-center">
        ...
        >
          <path
            ...
            stroke-linejoin="round"
            stroke-width="2"
            d="M8 4h10a2 2 0 012 2v12a2 2 0 01-2 2H8m0-16a2 2 0 00-2 2v12a2 2 0 002 2m0-16v16m4-8h4"
          />
        </svg>
        Visa Requirements
      </h2>
      <p class="text-lg text-gray-800 dark:text-gray-300" v-if="visaReq.data">
        Visa Type: <span class="font-medium">{{ typeOfVisa }}</span
        ><br />
        Duration: <span class="font-medium">{{ visaDuration }}</span
        ><br />
    ...
    <div class="p-4 bg-white dark:bg-gray-700 rounded shadow-md">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 mt-4">
        <div
          v-for="(temp, day) in sevenDayWeather.temps"
          :key="day"
